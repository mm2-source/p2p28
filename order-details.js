...
(function () {
  ...
  async function fetchAndShowSellerPayment(order) {
    ...
    // Only show payment details to the buyer
    ...
    try {
      ...
      // Try shared helper first (available when profile.js is loaded on same page)
      if (typeof window.P2P?.getSellerPaymentInfo === "function") {
        ...
        // Direct Firestore query — self-contained fallback for order-details.html
        ...
        if (doc.exists) {
          ...
          // Match by payment method type first, then fall back to first method
          let matched = null;
          ...
          if (primary) {
            info = { name, paymentMethods: methods, primaryMethod: primary };
          }
        }
      }

      if (!info || !info.primaryMethod) {
        // Seller has no saved payment methods — show fallback
